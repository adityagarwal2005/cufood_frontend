...
function renderLocations(locations) {
  ...
  return locations.map((location, index) => {
    ...
    const photoLayer = hasPhoto
      ? `
        <span class="absolute inset-0 bg-cover bg-center transition-transform duration-200 group-hover:scale-110" style="background-image:url('${escapeHtml(location.photo)}')"></span>
        <span class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/25 to-black/10"></span>
      `
      : `
        <span class="absolute -top-10 -right-10 w-36 h-36 rounded-full bg-white/10 blur-2xl transition-transform duration-200 group-hover:scale-125"></span>
        <span class="absolute -bottom-16 -left-10 w-44 h-44 rounded-full bg-white/10 blur-2xl"></span>
      `;

    card.innerHTML = `
      ${photoLayer}
      <span class="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-500 ease-out bg-gradient-to-r from-transparent via-white/10 to-transparent skew-x-12"></span>

...
async function loadLocationStat(slug, statEl) {
  try {
    ...
    statEl.textContent = `${restaurants.length} ${outletWord} · ${openCount} open now`;
  } catch (err) {
    statEl.textContent = "";
    console.error(err);
  }
...
function renderLocations(locations) {
  ...
  locations.forEach((location, index) => {
    ...
    card.className =
      `group relative flex flex-col items-center gap-4 overflow-hidden bg-gradient-to-br ${theme} rounded-3xl shadow-lg hover:shadow-2xl hover:-translate-y-2 active:translate-y-0 transition-all duration-300 px-8 py-12 sm:px-10 sm:py-16 text-inherit no-underline`;
    ...
    card.innerHTML = `
      ...
      <span class="relative flex flex-col items-center gap-1.5">
        <span class="text-xl sm:text-2xl font-extrabold text-white tracking-tight">${escapeHtml(location.name)}</span>
        <span class="text-xs font-semibold text-white/70" data-location-stat>Loading outlets&hellip;</span>
        <span class="inline-flex items-center gap-1 text-xs font-semibold text-white/90 uppercase tracking-wide mt-1.5">
          Explore outlets
    ...
    loadLocationStat(location.slug, card.querySelector("[data-location-stat]"));
  });
}
